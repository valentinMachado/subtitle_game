async function main(socketUrl) {
  ...
  const subtitleOverlayLocal = document.getElementById("subtitleOverlayLocal");
  ...
  const previousTimeButton = document.getElementById("previousTimeButton");
  const nextTimeButton = document.getElementById("nextTimeButton");

  const emoticonActionMenu = document.getElementById("emoticonActionMenu");
    ...
    savesList.innerHTML = saves
      .map((s, index) => {
        ...
        <div class="saveItem" style="display:flex; justify-content:space-between; align-items:center;">
          <span>🤖📄   ${s.name}</span>
          <div>
            <button onclick="loadSave(${index})">✍</button>
            <button onclick="deleteSave(${index})">❌</button>
            <button onclick="selectClipToInsert('${state.video.id}', ${index})">➕→🕔</button>
          </div>
        </div>
      `;
      })
  ...
  const renderLocalOverlay = () => {
    ...
    const active = playerSubtitles.find((s) => t >= s.start && t <= s.end);
    ...
    if (active && !isRemoteVisible) {
      subtitleOverlayLocal.textContent = active.text || active.placeholder;
      subtitleOverlayLocal.style.display = "block";
      if (!isEditingSubtitle) scrollToSub(playerSubtitles.indexOf(active));
  ...
  const updateHeight = () => {
    ...
    const subtitleFormHeight = subtitleForm.getBoundingClientRect().height;
  ...
  const displayView = (id) => {
    ...
    if (id === "playerView" || id === "remoteView") {
      ...
      if (Object.values(libraryVideosById).length > 0) {
        libraryVideo.src =
          libraryVideosById[librarySelect.value].path + `?token=${token}`;
        libraryVideoTrack.src =
          libraryVideosById[librarySelect.value].srt.replace(
            /\.srt$/i,
            ".vtt"
          ) + `?token=${token}`;
      }
    ...
    deleteClipButton.style.display = id === "libraryView" ? "block" : "none";
  ...
  window.addEventListener("resize", () => {
    ...
    }, 50); // 50ms après le dernier resize, la hauteur est stabilisée
  });
  ...
  function hideOtherSubtitles(i) {
    document
      .querySelectorAll(".subtitleItem")
      .forEach((el, j) => (el.style.display = j >= i - 1 ? "flex" : "none"));
  }
  ...
  function scrollToSub(i, duration = 0, smooth = true) {
    ...
    // Annule un scroll précédent si actif
    if (scrollTimeout) {
  ...
  const updateVideoState = async (videoState) => {
    if (videoState.playing) {
      ...
      videoStateButton.textContent = "⏸️";
    } else {
      gameVideo.pause();
      videoStateButton.textContent = "▶️";
    }
  ...
  // Sélection d'un clip depuis la liste des saves
  window.selectClipToInsert = function selectClipToInsert(videoId, saveIndex) {
  ...
  // Render timeline avec slots ➕ uniquement si un clip est sélectionné
  function renderTimeline() {
    ...
    labelStart.style.flex = "0 0 auto"; // ne pas étirer
    ...
    timelineClips.forEach((clip, i) => {
      ...
      div.style.overflow = "hidden"; // empêche le texte de dépasser
      ...
      delBtn.textContent = "❌";
      delBtn.style.flex = "0 0 auto"; // ne pas étirer
      delBtn.onclick = (e) => {
        e.stopPropagation();
    ...
    function addSlot(position) {
      ...
      slot.onclick = () => {
        ...
        selectedClipToInsert = null; // désélection
        renderTimeline();
        socket.emit("updateTimeline", timelineClips);
      };
  ...
  const receivedSound = new Audio("/sounds/notif.wav" + `?token=${token}`);
  ...
  socket.on("gameState", async (state) => {
    ...
    if (state.receivedSubtitles) {
      ...
      if (currentViewId == "playerView") {
        submitButton.classList.remove("pulse-once"); // reset au cas où
    ...
    if (state.finishedRender) {
      goToRender.classList.remove("pulse-once"); // reset au cas où
      void goToRender.offsetWidth; // force reflow
      goToRender.classList.add("pulse-once");
    }

    // Reset selectedClipToInsert si clip plus présent
    ...
    timelineClips = structuredClone(state.timeline);
    ...
    Object.entries(state.emoticons).forEach(([name, text]) => {
      ...
      if (!currentEmoticons[name]) {
        const createEmoticon = () => {
          ...
          const y =
            -Math.random() * videoRect.height * 0.5 + videoRect.bottom - 50;
    ...
    playersList.innerHTML = Object.entries(state.players)
      .map(
        ([id, p]) =>
          ...
          }" onclick="selectPlayer('${id}')"><span>🤖</span><span>${
            p.submitted ? "📄" : "❌"
          }</span>
                                  </span><span>${
  ...
  function renderLibraryView() {
    ...
    if (libraryVideos.length) {
      ...
      // Regrouper les vidéos par langue
      const videosByLang = {};
      ...
      Object.keys(videosByLang)
        .sort() // trier les langues par ordre alphabétique
        .forEach((lang) => {
          const optgroup = document.createElement("optgroup");
          optgroup.label = lang;

          // trier les vidéos par id dans chaque langue
          videosByLang[lang]
            .sort((a, b) => a.id.toString().localeCompare(b.id.toString()))
            .forEach((v) => {
              const option = document.createElement("option");
    ...
    if (!clips.length) {
      ...
      o.textContent = "Aucun clip créer";
      o.disabled = true;
      o.selected = true;
  ...
  librarySelect.onchange = () => {
    libraryVideo.src =
      libraryVideosById[librarySelect.value].path + `?token=${token}`;
    libraryVideoTrack.src =
      libraryVideosById[librarySelect.value].srt.replace(/\.srt$/i, ".vtt") +
      `?token=${token}`;
  };
  ...
  libraryVideo.addEventListener("timeupdate", () => {
    ...
    // Si le temps courant dépasse la fin, pause la vidéo
  ...
  createBtn.addEventListener("click", () => {
    ...
    if (!startInput.checkValidity() || !endInput.checkValidity()) {
      // Force l’affichage du message du navigateur
    ...
    if (endSec - startSec < 5) {
      alert("La durée minimale est de 5 secondes");
      return;
    }

    if (endSec - startSec > 60 * 5) {
      alert("La durée maximale est de 5 minutes");
      return;
    }
  ...
  // clic global → fermer tous les menus
