...
// Assurer que secret.users existe
if (!secret.users || typeof secret.users !== "object") {
  secret.users = {};
}

// ---------- Ajouter le user ----------
if (secret.users[USER_ID]) {
  console.warn(`⚠️ Le joueur "${USER_ID}" existe déjà. Écrasement.`);
}
...
try {
  ...
  console.log(`✅ User "${USER_ID}" ajouté avec succès !`);
} catch (err) {
